...
_L4:
    ...
    private static ServiceInfo zzb(Context context, Intent intent)
    {
        ...
        {
            Log.w("ConnectionTracker", (new StringBuilder()).append("There are no handler of this intent: ").append(intent.toUri(0)).toString());
            return null;
        }
        if (context.size() > 1)
        {
            Log.w("ConnectionTracker", (new StringBuilder()).append("There are multiple handlers for this intent: ").append(intent.toUri(0)).toString());
