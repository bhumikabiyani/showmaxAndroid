...
public class zzei
{
    ...
    public boolean zza(Context context, zzek zzek1, zzes zzes1)
    {
        ...
        if (!TextUtils.isEmpty(zzek1.zztS))
        {
            ...
            if (as.length < 2)
            {
                zzhx.zzac((new StringBuilder()).append("Could not parse component name from open GMSG: ").append(zzek1.zztS).toString());
                return false;
            }
        ...
        try
        {
            zzhx.zzab((new StringBuilder()).append("Launching an intent: ").append(intent.toURI()).toString());
