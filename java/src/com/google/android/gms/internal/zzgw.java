...
public final class zzgw extends zzgs.zza
{
    ...
    public static zzgq zza(Context context, String s, String s1, String s2, zzha zzha1)
    {
        ...
        {
            zzgz1 = new zzgz();
            zzhx.zzY((new StringBuilder()).append("AdRequestServiceImpl: Sending request: ").append(s1).toString());
            s1 = new URL(s1);
        ...
        {
            zzhx.zzac((new StringBuilder()).append("Error while connecting to ad server: ").append(context.getMessage()).toString());
            return new zzgq(2);
        }
        i = 0;
        ...
        httpurlconnection.disconnect();
        return context;
        zzhx.zzac((new StringBuilder()).append("Received error HTTP response code: ").append(j).toString());
        context = new zzgq(0);
        httpurlconnection.disconnect();
        return context;
        zzgz1.zzh(map);
    ...
    private static void zza(String s, Map map, String s1, int i)
    {
        if (zzhx.zzA(2))
        {
            zzhx.zzab((new StringBuilder()).append("Http Response: {\n  URL:\n    ").append(s).append("\n  Headers:").toString());
            if (map != null)
            {
                for (s = map.keySet().iterator(); s.hasNext();)
                {
                    Object obj = (String)s.next();
                    zzhx.zzab((new StringBuilder()).append("    ").append(((String) (obj))).append(":").toString());
                    obj = ((List)map.get(obj)).iterator();
                    while (((Iterator) (obj)).hasNext()) 
                    {
                        String s2 = (String)((Iterator) (obj)).next();
                        zzhx.zzab((new StringBuilder()).append("      ").append(s2).toString());
                    }
                }
            ...
            zzhx.zzab((new StringBuilder()).append("  Response Code:\n    ").append(i).append("\n}").toString());
        }
    }
