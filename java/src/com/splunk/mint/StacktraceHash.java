...
class StacktraceHash
{
    ...
    public static final HashMap manipulateStacktrace(String s, String s1)
    {
        ...
        hashmap.put("message", ((String) (obj1)).replaceAll("\n", " ").replaceAll("Caused by:", ""));
        ...
        hashmap.put("where", s);
        s1 = ((StringBuilder) (obj1)).toString().replaceAll("@\\w+", "").replaceAll(hexRegex, "").replaceAll(guidRegex, "").replaceAll("$\\w+", "");
        s = s1;
        if (!s2.contains(".java:"))
        {
            s = s1.replace(s2, s2.replaceAll("\\d+", ""));
        }
        hashmap.put("errorHash", md5(s));
        return hashmap;
    }
