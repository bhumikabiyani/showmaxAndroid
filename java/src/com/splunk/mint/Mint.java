...
public final class Mint
{
    ...
    public static ArrayList getURLBlackList()
    {
        return Properties.excludedUrls;
    }
...
_L2:
    ...
    public static void xamarinException(Exception exception, boolean flag, HashMap hashmap)
    {
        ...
        exception.printStackTrace(new PrintWriter(stringwriter));
        hashmap = new ExtraData(hashmap);
        exception = EnumExceptionType.UNHANDLED;
        if (flag)
        {
            exception = EnumExceptionType.HANDLED;
        }
        (new ActionError(EnumActionType.error, stringwriter.toString().replaceFirst("\n", "\n\t"), exception, hashmap)).save(new DataSaver());
    }
