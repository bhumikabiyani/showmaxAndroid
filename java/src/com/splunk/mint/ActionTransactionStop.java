...
//            Properties, TransactionsDatabase, Logger, DataSaver, 
//            NetSender
    ...
    implements InterfaceDataType
{
    ...
    public void save(DataSaver datasaver)
    {
        if (!goodForSave)
        {
            Logger.logError((new StringBuilder()).append(type.toString()).append(" without transaction start first!").toString());
            return;
        } else
        {
