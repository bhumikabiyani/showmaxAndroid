// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 

package com.mautilus.lib.videoview.ui;

import com.mautilus.lib.videoview.util.SystemUiHider;

// Referenced classes of package com.mautilus.lib.videoview.ui:
//            PlayerController

class this._cls0
    implements Runnable
{

    final PlayerController this$0;

    public void run()
    {
        if (PlayerController.access$000(PlayerController.this) != null && !PlayerController.access$100(PlayerController.this))
        {
            PlayerController.access$000(PlayerController.this).hide();
        }
    }

    ()
    {
        this$0 = PlayerController.this;
        super();
    }
}
