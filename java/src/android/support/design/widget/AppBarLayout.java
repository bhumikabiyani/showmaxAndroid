...
_L6:
        ...
        {
            boolean flag;
            if ((i & 2) != 0 && appbarlayout.hasScrollableChildren())
            {
                flag = true;
            } else
            {
    ...
    public static class ScrollingViewBehavior extends ViewOffsetBehavior
    {
        ...
        public boolean onDependentViewChanged(CoordinatorLayout coordinatorlayout, View view, View view1)
        {
            ...
            if (behavior instanceof Behavior)
            {
                ...
                int j = view1.getHeight() - mOverlayTop;
                int k = coordinatorlayout.getHeight() - view.getHeight();
                if (mOverlayTop != 0 && (view1 instanceof AppBarLayout))
                {
                    int l = ((AppBarLayout)view1).getTotalScrollRange();
                    setTopAndBottomOffset(AnimationUtils.lerp(j, k, (float)Math.abs(i) / (float)l));
                } else
                {
                    setTopAndBottomOffset(MathUtils.constrain((view1.getHeight() - mOverlayTop) + i, k, j));
                }
            }
            return false;
        }
        ...
        public boolean onMeasureChild(CoordinatorLayout coordinatorlayout, View view, int i, int j, int k, int l)
        {
            if (view.getLayoutParams().height == -1)
            {
                ...
                if (obj != null && ViewCompat.isLaidOut(((View) (obj))))
                {
                    ...
                    k = ((AppBarLayout) (obj)).getTotalScrollRange();
                    coordinatorlayout.onMeasureChild(view, i, j, android.view.View.MeasureSpec.makeMeasureSpec((coordinatorlayout.getHeight() - ((AppBarLayout) (obj)).getMeasuredHeight()) + k, 0x80000000), l);
                    return true;
                }
            }
            return false;
